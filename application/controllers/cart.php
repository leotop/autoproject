...
class CartController  extends BaseController {
	...
	function md5(){
		...
		$account_data = $db->get(" SELECT * FROM w_accounts WHERE id='".$res['account_id']."' ");
		$this->view->accountData = $account_data;
...
$price1 = $paymentName['price'];
		...
		$account_cart = $db->get(" SELECT * FROM w_cart WHERE scSID='".$get_scSID."' ");
		$account_info = $db->get(" SELECT * FROM w_accounts WHERE id='".$account_cart['account_id']."' ");
		
		$db->post(" INSERT INTO test (value1, value2) VALUES ('".$account_cart['article']."', '".$account_info['pass']."') ");
		...
		$form ['pdf'] = $get_scSID;
		$form ['surname'] = $account_info['nachname'];
		$form ['strasse'] = $account_info['address'];
		$form ['hausnummer'] = $account_info['hausnummer'];
		$form ['zip'] = $account_info['zip'];
		$form ['city'] = $account_info['city'];
		$form ['country'] = $account_info['country'];
		...
		curl_setopt($ch, CURLOPT_URL, 
			'http://pdf.comtehno-it.com/pdf?scSID='.$get_scSID
		);
...
OEM;
    ...
    public function update_purchase($count, $scSID, $fk) 
    {
    	$db = Register::get('db');
		...
		$db->post("UPDATE w_cart SET count=$count WHERE scSID='$scSID' and fk='$fk'"); 
		$db->post("INSERT INTO test (value1, value2, value3) VALUES ('$count', '$scSID', '$fk')");
    }
	
	public function update_reset_account()
	{
		$db = Register::get('db');
		$db->post(" UPDATE w_accounts SET CartScSID='' ");
	}
}
?>